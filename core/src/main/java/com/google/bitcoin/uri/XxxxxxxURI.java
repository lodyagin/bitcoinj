...
/**
 * <p>Provides a standard implementation of a Xxxxxxx URI with support for the following:</p>
 *
 * <ul>
 ...
 */
public class XxxxxxxURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(XxxxxxxURI.class);
    ...
    /**
     * Constructs a new XxxxxxxURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws XxxxxxxURIParseException if the URI is not syntactically or semantically valid.
     */
    public XxxxxxxURI(String uri) throws XxxxxxxURIParseException {
        this(null, uri);
    }

    /**
     * Constructs a new object by trying to parse the input as a valid Xxxxxxx URI.
     *
     ...
     *
     * @throws XxxxxxxURIParseException If the input fails Xxxxxxx URI syntax and semantic checks.
     */
    public XxxxxxxURI(@Nullable NetworkParameters params, String input) throws XxxxxxxURIParseException {
        checkNotNull(input);
        ...
        try {
            ...
            throw new XxxxxxxURIParseException("Bad URI syntax", e);
        }
        ...
        if (input.startsWith("bitcoin://")) {
            ...
            throw new XxxxxxxURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?");
        if (addressSplitTokens.length == 0)
            throw new XxxxxxxURIParseException("No data found after the bitcoin: prefix");
        ...
        if (addressSplitTokens.length == 1) {
            ...
            if (addressSplitTokens.length == 2) {
                ...
                throw new XxxxxxxURIParseException("Too many question marks in URI '" + uri + "'");
            }
        }
        ...
        if (!addressToken.isEmpty()) {
            // Attempt to parse the addressToken as a Xxxxxxx address for this network
            try {
                Address address = new Address(params, addressToken);
                putWithValidation(FIELD_ADDRESS, address);
            } catch (final AddressFormatException e) {
                throw new XxxxxxxURIParseException("Bad address", e);
            }
        }

        if (addressToken.isEmpty() && getPaymentRequestUrl() == null) {
            throw new XxxxxxxURIParseException("No address and no r= parameter found");
        }
    }
     ...
     */
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws XxxxxxxURIParseException {
        // Attempt to decode the rest of the tokens into a parameter map.
        for (String nameValuePairToken : nameValuePairTokens) {
            final int sepIndex = nameValuePairToken.indexOf('=');
            if (sepIndex == -1)
                throw new XxxxxxxURIParseException("Malformed Xxxxxxx URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new XxxxxxxURIParseException("Malformed Xxxxxxx URI - empty name '" +
                        nameValuePairToken + "'");
     ...
     */
    private void putWithValidation(String key, Object value) throws XxxxxxxURIParseException {
        if (parameterMap.containsKey(key)) {
            throw new XxxxxxxURIParseException(String.format("'%s' is duplicated, URI is invalid", key));
        } else {
    ...
    /**
     * The Xxxxxxx Address from the URI, if one was present. It's possible to have Xxxxxxx URI's with no address if a
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("XxxxxxxURI[");
        boolean first = true;
    ...
    public static String convertToXxxxxxxURI(Address address, BigInteger amount, String label, String message) {
        return convertToXxxxxxxURI(address.toString(), amount, label, message);
    }

    /**
     * Simple Xxxxxxx URI builder using known good fields.
     * 
     * @param address The Xxxxxxx address
     * @param amount The amount in nanocoins (decimal)
     * @param label A label
     * @param message A message
     * @return A String containing the Xxxxxxx URI
     */
    public static String convertToXxxxxxxURI(String address, @Nullable BigInteger amount, @Nullable String label,
